...
ldef.define('keyword', 'if else _ break continue returns return while'.split(' '));
ldef.define('keyword', 'enum struct import abstract contract interface library modifier event'.split(' '));
ldef.define('keyword', 'function constructor public private external internal virtual override'.split(' '));
ldef.define('keyword', 'payable view pure'.split(' '));
ldef.define('keyword', 'constant as is from memory storage'.split(' '));
ldef.define('keyword', 'bool address string fixed ufixed mapping'.split(' '));
ldef.define('keyword', 'byte bytes'.split(' '));
...
pdef.define('constantdecl', [ 'type', 'keyword:constant', 'name:', 'operator:=', 'expression' ], function (values) { return geast.namedconstant(values[2], values[0], values[4]); });

// import
pdef.define('import', [ 'keyword:import', 'string:', 'keyword:as', 'name:', 'delimiter:;' ], function (values) { return geast.import(values[1], values[3]); });
pdef.define('import', [ 'keyword:import', 'string:', 'delimiter:;' ], function (values) { return geast.import(values[1]); });
pdef.define('import', [ 'keyword:import', 'operator:*', 'keyword:as', 'name:', 'keyword:from', 'string:', 'delimiter:;' ], function (values) { return geast.import(values[5], values[3]); });
...
pdef.define('contract', [ 'keyword:abstract', 'keyword:contract', 'name:', 'keyword:is', 'inheritance-list', 'delimiter:{', 'contdecllist', 'delimiter:}' ], function (values) { return geast.contract(values[2], geast.sequence(values[6]), values[4], true); });
pdef.define('contract', [ 'keyword:contract', 'name:', 'keyword:is', 'inheritance-list', 'delimiter:{', 'contdecllist', 'delimiter:}' ], function (values) { return geast.contract(values[1], geast.sequence(values[5]), values[3]); });
...
pdef.define('interface', [ 'keyword:interface', 'name:', 'keyword:is', 'inheritance-list', 'delimiter:{', 'contdecllist', 'delimiter:}' ], function (values) { return geast.interface(values[1], geast.sequence(values[5]), values[3]); });
...
// if command
pdef.define('command', [ 'keyword:if', 'delimiter:(', 'expression', 'delimiter:)', 'command', 'keyword:else', 'command' ], function (values) { return geast.conditional(values[2], values[4], values[6]); });
...
// underscore command
pdef.define('command', [ 'keyword:_', 'delimiter:;'], function (values) { return geast.underscore(); });
// composite command
...
pdef.define('simpletype', 'keyword:byte');
pdef.define('simpletype', 'keyword:bytes');

for (let k = 1; k <= 32; k++)
    pdef.define('simpletype', 'keyword:bytes' + k);
