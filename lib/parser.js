...
ldef.define('keyword', 'if break continue returns return'.split(' '));
ldef.define('keyword', 'enum struct import abstract'.split(' '));
...
pdef.define('contract', [ 'keyword:abstract', 'name:contract', 'name:', 'delimiter:{', 'contdecllist', 'delimiter:}' ], function (values) { return geast.contract(values[2], geast.sequence(values[4]), null, true); });
pdef.define('contract', [ 'keyword:abstract', 'name:contract', 'name:', 'name:is', 'inheritance-list', 'delimiter:{', 'contdecllist', 'delimiter:}' ], function (values) { return geast.contract(values[2], geast.sequence(values[6]), values[4], true); });
...
// library
pdef.define('library', [ 'name:library', 'name:', 'delimiter:{', 'contdecllist', 'delimiter:}' ], function (values) { return geast.library(values[1], geast.sequence(values[3])) });

// event
pdef.define('event', [ 'name:event', 'name:', 'delimiter:(', 'arglist', 'delimiter:)' ], function (values) { return geast.event(values[1], values[3]); });

// modifier
pdef.define('modifier', [ 'name:modifier', 'name:', 'delimiter:(', 'arglist', 'delimiter:)', 'composite-command' ], function (values) { return geast.modifier(values[1], values[3], values[5]); }); 
...
pdef.define('function-attributes', [ 'name:virtual', 'function-attributes' ], function(values) { values[1].virtual = true; return values[1]; });
pdef.define('function-attributes', [ 'name:override', 'function-attributes' ], function(values) { values[1].override = true; return values[1]; });
...
pdef.define('simpletype', 'name:uint');

for (let k = 8; k <= 256; k += 8)
    pdef.define('simpletype', 'name:uint' + k);
    
pdef.define('simpletype', 'name:int');

for (let k = 8; k <= 256; k += 8)
    pdef.define('simpletype', 'name:int' + k);
...
pdef.define('simpletype', 'name:bool');
pdef.define('simpletype', 'name:address');
pdef.define('simpletype', 'name:string');
pdef.define('simpletype', 'name:fixed');
pdef.define('simpletype', 'name:ufixed');
pdef.define('mapping', [ 'name:mapping', 'delimiter:(', 'simpletype', 'operator:=>', 'type', 'delimiter:)' ], function (values) { return geast.mapping(values[2], values[4]); });
